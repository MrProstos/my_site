...
$result_word = $words->Select(["а"]); //Берем данные из представления которое объеденяет таблицу article и words
if (!$result_word) {
...
foreach ($result_article as $item => $value) {
    ...
    $article_words->Insert([$result_article[0]["id"],$result_word[0]["id"],$cnt]);
}
